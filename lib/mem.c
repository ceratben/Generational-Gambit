...
/* words occupied by objects in old gen */
___HIDDEN long words_old;

/* words occupied in heap by movable objects excluding current msections */
___HIDDEN long words_prev_msections;

/* A tag bit for the nursery's remset and SSB operation. 
 * ___MOVABLE2 is not currently used so we reused this bit.
 */
#define ___TAG0 ___MOVABLE2

/* Number of msections in the nursery */
#define ___NB_NURSERY_MSECTION 1
#define ___REMSET_SIZE (___MSECTION_SIZE >> 2)
#define ___SSB_SIZE (1024 * ___WS)

/* words occupied in heap by movable objects including current msections */
#define WORDS_MOVABLE0 \
(2*((alloc_stack_start - alloc_stack_ptr) + \
(alloc_heap_ptr - alloc_heap_start)))

#define WORDS_MOVABLE (WORDS_MOVABLE0 + words_old)
...
#define WORDS_MOVABLE1_USABLE \
(2*the_msections->nb_sections*(long)((___MSECTION_SIZE>>1)-___MSECTION_FUDGE+1))

#define WORDS_MOVABLE0_USABLE \
(2*the_nursery->nb_sections*(long)((___MSECTION_SIZE>>1)-___MSECTION_FUDGE+1))

#define WORDS_MOVABLE_USABLE WORDS_MOVABLE0_USABLE
...
/* Indicate to collect all of memeory */
___HIDDEN ___BOOL full_collect_flag;

#define NEED_FULL_COLLECT 1//((WORDS_MOVABLE1_USABLE - words_old) < WORDS_MOVABLE0)
...
___HIDDEN msections *the_nursery;

/* the remembered set of the nursery */
___HIDDEN buffer *remset;
buffer *SSB;

/* location of tospace in each msection */
___HIDDEN ___BOOL tospace_at_top;
___HIDDEN ___BOOL nursery_tospace_at_top;

/* location of old pointer in each generation */
___HIDDEN ___WORD *nursery_old_ptr;
___HIDDEN ___WORD *msection_old_ptr;
...
int kind;)
{
  ...
  //printf("alloc_scmobj: WORDS_OCCUPIED = %i, heap_size = %i \n", WORDS_OCCUPIED, heap_size);
...
___P((msection *s, ___BOOL t),
     (s, t)
msection *s; ___BOOL t)
{
  if (t)
    return s->base;
  else
...
{
  printf("next_stack_msection\n");
  if (stack_msection != 0)
    words_prev_msections += alloc_stack_start - alloc_stack_ptr;

  stack_msection = next_msection (heap_msection);
  alloc_stack_limit = start_of_tospace (stack_msection, nursery_tospace_at_top);
...
___HIDDEN void next_heap_msection ___PVOID
{
  ...
  alloc_heap_start = start_of_tospace (heap_msection, nursery_tospace_at_top);
  alloc_heap_limit = alloc_heap_start + (___MSECTION_SIZE>>1);
  alloc_heap_ptr = alloc_heap_start;
}

___HIDDEN void next_old_msection ___PVOID
{
  printf("next_old_msection: %p\n", old_msection);
  if (old_msection != 0)
    {
      words_prev_msections += alloc_old_ptr - alloc_old_start;
      old_msection->alloc = alloc_old_ptr;
      old_msection = next_msection (old_msection);
    }
  else
    {
      /* set alloc_msection to the second msection of the_msections */
      alloc_msection = the_msections->sections[0];
      old_msection = alloc_msection;
      nb_msections_used++;
    }

  printf("alloc_msection->next: %p\n", alloc_msection->next);
  printf("after next_msection: old_msection = %p\n", old_msection);

  alloc_old_start = start_of_tospace (old_msection, tospace_at_top);
  alloc_old_limit = alloc_old_start + (___MSECTION_SIZE>>1);
  alloc_old_ptr = alloc_old_start;
}
...
___HIDDEN void add_remset ___P((___WORD *ptr), (ptr) ___WORD *ptr;)
{
  printf("Begin add_remset\n");
  /* Add the ptr to the remset if it isn't already there. */
  ___WORD head = ptr[-1];
  ___WORD head_typ = ___HD_TYP(head);

  if (head_typ == ___MOVABLE0)
    {
      *(remset->ptr++) = ptr;

      /* Tag the object to indicate it is pointed to */
      ptr[-1] = head + ___TAG0;
    }
  printf("End add_remset\n");
}

___HIDDEN void remove_remset ___P((___WORD **ptr), (ptr) ___WORD **ptr;)
{
  /* Overwrite the ptr with the last element */
  if (ptr != remset->start)
    {
      *ptr = *(remset->ptr--);
    }
  /* Remove the tag from the pointed object. 
   * Assuming it has one and is in the nursery. 
   */
  ptr[-1] = ptr[-1] - ___TAG0;

}

___HIDDEN void clear_remset ___PVOID
{

  ___WORD **current = remset->ptr, **limit = remset->start;
  ___WORD head, head_typ;

  /* Iterate on all pointers in the remset. */
  while (current-- > limit)
    {  
      head = (*current)[-1];
      ...
      if (head_typ == ___FORW)
	{
      ...
      if (head_typ == ___TAG0) continue;
      else
	{
	  /* Pointer is obselete. */
	  remove_remset (current);
	}
    }
}
...
/* Scan obj to see if it contains any backpointers. If it does, add the pointed object to remset. */
___HIDDEN void manage_SSB ___P((___WORD *obj), (obj) ___WORD *obj;)
{
  printf("Begin manage_SSB: obj = %p\n", obj);
  ___WORD head = obj[-1];
  ___WORD gen = ___HD_TYP(head);
  ___WORD len = ___HD_WORDS(head);
  ___WORD *this;

  while(len--)
    {
      this = (___WORD *)obj[len];

      if (((___WORD) this) & 0x1) /* this is a pointer */
	{	  
	  /* Test if within nursery */
	  if ((this >= alloc_heap_start) && (this <= alloc_heap_limit))
	    {
	      add_remset (this);
	    }
	}
...
/* Call manage_SSB on every pointer in the SSB to empty it. */
___HIDDEN void clear_SSB ___PVOID
{
  ___WORD **current = SSB->ptr, **limit = SSB->start;
  ___WORD head, head_typ;

  while (current-- > limit)
    {
      /* Remove ___MOVABLE1 tags from nursery objects. */
      head = (*current)[-1];
      head_typ = ___HD_TYP(head);
      if (head_typ == ___MOVABLE1)
	{
	  (*current)[-1] = head - ___MOVABLE1 + ___MOVABLE0; 
	}
...
/* Remove all duplicate from SSB. If insufficient, call clear_SSB */
void clean_SSB ___PVOID
{
  ___WORD **current = SSB->start, **limit = SSB->ptr;
  ___WORD **next = current;
  ___WORD head, head_typ;
  while (current < limit)
    {
      head = (*current)[-1];
      head_typ = ___HD_TYP(head);
      /* If the object is not already tagged and not already present */
      if (head_typ == ___MOVABLE0)
	{
	  *(next++) = *current;
	  /* Set its header to ___MOVABLE1 */
	  (*current)[-1] = head + ___MOVABLE1 - ___MOVABLE0;
	}
      current++;
    }

  /* Clear if sufficient space is not recovered */
  if (0)
    {
      clear_SSB(); 
    }
...
void add_SSB ___P((___WORD *ptr), (ptr) ___WORD *ptr;)
{
  *(SSB->ptr++) = ptr;
}
      ...
      if (___MEM_ALLOCATED(obj))
        {
          ...
          if ((head_typ == ___MOVABLE0) || (head_typ == ___TAG0) || (full_collect_flag && head_typ == ___MOVABLE1) )
            {
              long words = ___HD_WORDS(head);
	      int dest = ___MOVABLE0;

	      if (head_typ == ___MOVABLE0)  
		{
		  if ((___WORD*) obj < nursery_old_ptr)
		    {
		      /* Promote this object to old */
		      //printf("Promoting: start = %p, nursery_old_ptr = %p\n", obj, nursery_old_ptr);
		      alloc = alloc_old_ptr;
		      limit = alloc_old_limit;
		      words_old += words;
		      dest = ___MOVABLE1;
		    }
		  else
	      ...
	      else if (head_typ == ___MOVABLE1)
		{
		  //		  if ((___WORD*) obj < msection_old_ptr)
		  //{
		      /* Allocate to stills */
		  //  printf("Need to move to stills\n");
		      //words_old -= words;
		      //dest = 2;
		  //  alloc = alloc_old_ptr;
		  //  limit = alloc_old_limit;
...
#if ___WS == 4
              ___BOOL pad = 0;
              while ((dest == ___MOVABLE1) && 
		     (alloc + words + (subtype >= ___sS64VECTOR ? 2 : 1) >
		      limit))
#else
		while ((dest == ___MOVABLE1) && (alloc + words + 1 > limit))
#endif
                {
                  alloc_old_ptr = alloc;
		  printf("Here: alloc: %p, limit: %p\n", alloc + words +1, limit);
                  next_old_msection ();
                  alloc = alloc_old_ptr;
                  limit = alloc_old_limit;
                }
              ...
              if (subtype >= ___sS64VECTOR)
                {
                  if ((___CAST(___WORD,alloc) & (8-1)) == 0)
                    *alloc++ = ___MAKE_HD(0, ___sVECTOR, dest);
                  else
                    pad = 1;
                }
...
#endif
	      /* Promote head if necessary */
	      if ((dest == ___MOVABLE1) && 
		  (head_typ == ___MOVABLE0)) 
		{
		  head = ___MAKE_HD((words * ___WS),subtype, ___MOVABLE1);
		  body[-1] = head;
		  manage_SSB(body);
		}
	      ...
	      /* Move the relevant values for alloc_x_ptr */

	      if (dest == ___MOVABLE0) alloc_heap_ptr = alloc;
	      else if (dest == ___MOVABLE1) alloc_old_ptr = alloc;
...
___WORD *orig_ptr;)
{
  ...
  ___WORD ra2;
  ___WORD cf;
  printf("In mark_captured_continuation\n");
  cf = *ptr;
...
fp=0x1006fff68 ra1=0x1001f9bc1 fs=3 link=0
      */
	  ...
	  printf("___SUBTYPED_OVERHEAD: %i\n", ___SUBTYPED_OVERHEAD);
          while (alloc + words + ___SUBTYPED_OVERHEAD > limit)
            {
	      printf("Executing this: alloc = %p\n", alloc);
              alloc_heap_ptr = alloc;
              next_heap_msection ();
              alloc = alloc_heap_ptr;
              limit = alloc_heap_limit;
...
{
  printf("Begin scan_movable_objs_to_scan\n");
  ___WORD *body;
  long words;

  for (;;)
    {
      if (scan_old_msection == old_msection)
        {
          if (old_scan_ptr >= alloc_old_ptr)
	    {
	      break;
	    }
        }
      else if (old_scan_ptr >= scan_old_msection->alloc)
        {
          scan_old_msection = scan_old_msection->next;
          old_scan_ptr = start_of_tospace (scan_old_msection, tospace_at_top);
          continue;
        }
      body = old_scan_ptr + 1;
      words = scan (body);
      old_scan_ptr = body + words;
    };
  printf("End scan_movable_objs_to_scan\n");
}

___HIDDEN void scan_nursery_obj_to_scan ___PVOID
{
  ___WORD *body;
  long words;

  while (scan_ptr < alloc_heap_ptr) 
    {
      body = scan_ptr + 1;
      words = scan (body);
      scan_ptr = body + words;
...
___HIDDEN void scan_remset ___PVOID
{
  ___WORD ** current = remset->start;

  while (current != remset->ptr)
    {
      scan (*current++);
    }
...
___SCMOBJ ___setup_mem ___PVOID
{
  ...
  printf("___MIN_NB_MSECTIONS: %i\n", ___MIN_NB_MSECTIONS); 

  if (init_nb_sections < ___MIN_NB_MSECTIONS + ___NB_NURSERY_MSECTION)
    init_nb_sections = ___MIN_NB_MSECTIONS + ___NB_NURSERY_MSECTION;

  adjust_msections (&the_msections, init_nb_sections);
  adjust_msections (&the_nursery, ___NB_NURSERY_MSECTION);

  /* Allocate remset and SSB */
  remset = (buffer *) malloc(sizeof(buffer) + ___REMSET_SIZE);
  SSB = (buffer *) malloc(sizeof(buffer) + ___SSB_SIZE);
  ...
  if (the_nursery == 0 ||
      the_nursery->nb_sections != ___NB_NURSERY_MSECTION)
    return ___FIX(___HEAP_OVERFLOW_ERR);
  ...
  /* Initialize SSB and remset fields */
  remset->start = remset->set;
  remset->limit = remset->start + ___REMSET_SIZE;
  remset->ptr = remset->start;

  SSB->start = SSB->set;
  SSB->limit = SSB->start + ___SSB_SIZE;
  SSB->start = SSB->start;
...
___HIDDEN void free_buffer ___P((buffer *this), (this) buffer *this;)
{
  free(this->set);
  free(this);
}
...
#endif
{
  ...
  printf("NEED_FULL_COLLECT: %i\n", NEED_FULL_COLLECT);
  full_collect_flag = NEED_FULL_COLLECT;
  //msection_old_ptr = 0;// alloc_old_ptr;
  //printf("alloc_heap_ptr: %p, nursery_old_ptr: %p\n",  alloc_heap_ptr, nursery_old_ptr);
  //nursery_old_ptr = alloc_heap_ptr;

  alloc_stack_ptr = ___ps->fp; /* needed by 'WORDS_OCCUPIED' */
  alloc_heap_ptr  = ___ps->hp; /* needed by 'WORDS_OCCUPIED' */
  
  //printf("alloc_stack_start - alloc_stack_ptr: %p\n", (___ps->stack_break - alloc_stack_ptr));
  //printf("heap_size: %i\n", heap_size);
  //printf("stack_msection: %p, heap_msection: %p\n", stack_msection, heap_msection);
  ...
  nursery_tospace_at_top = !nursery_tospace_at_top;
  stack_msection = 0;
  heap_msection = 0;
  nb_msections_used = 0;
  
  if (full_collect_flag)
    {
      tospace_at_top = !tospace_at_top;
      old_msection = the_msections->head;
    }

  next_heap_msection ();

  //printf("After next_heap_msection\n");

  scan_msection = heap_msection;
  scan_old_msection = old_msection;
  scan_ptr = alloc_heap_ptr;
  old_scan_ptr = alloc_old_ptr;
  ...
  if (!full_collect_flag) scan_remset();

  again:

  if ((___CAST(___WORD*,still_objs_to_scan) != 0) && full_collect_flag)
    {
      printf("Stills: ahp = %p\n", alloc_heap_ptr);     
      scan_still_objs_to_scan ();
    }
  /* Scan the nursery */
  printf("scan_ptr: %p, alloc_heap_ptr: %p\n",scan_ptr, alloc_heap_ptr);
  if ((scan_msection != heap_msection) ||
      (scan_ptr < alloc_heap_ptr))
    {
      printf("Nursery: ahp = %p\n", alloc_heap_ptr);  
      scan_nursery_obj_to_scan ();
      goto again;
    }

  if (full_collect_flag && ((scan_old_msection != old_msection) ||
			    (old_scan_ptr < alloc_old_ptr)))
    {
      printf("Old: scan_old_msection = %p, old_msection = %p\n", scan_old_msection, old_msection);
      printf("Old: old_scan_ptr = %p, alloc_old_ptr = %p\n", old_scan_ptr, alloc_old_ptr);
  ...
  if (target_nb_sections < nb_msections_used+1)
    {
      target_nb_sections = the_msections->nb_sections+1;
      overflow = 1;
    }

  if (target_nb_sections < ___MIN_NB_MSECTIONS+1)
    target_nb_sections = ___MIN_NB_MSECTIONS+1;
...

  {
    ...
    /*     p1 = start + length; */
    /*     p2 = start_of_fromspace (the_msections->head, nursery_tospace_at_top) + length; */
